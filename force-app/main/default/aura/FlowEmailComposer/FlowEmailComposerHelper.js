({
    ...
    getRelatedFiles: function (cmp, evt, helper) {
        let action = cmp.get("c.fetchRelatedFiles");
        action.setParams(
            {
                "whatId": cmp.get("v.whatId")
            }
        );
        action.setCallback(this, function (res) {
            ...
            if (state === "SUCCESS") {
                ...
                if (errors) {
                    if (errors[0] && errors[0].message) {
                        console.log("Error fetching related files: " + errors[0].message);
                    }
                }
            } else {
                console.log("Unknown error whenevever fetching related files...");
            }
        });
    ...
    updateFileType: function (cmp, files) {
        console.log("Updating files:");
        console.log(files);
        let action = cmp.get("c.updateFiles");
        action.setParams({
            "files": files,
        ...
        action.setCallback(this, function (res) {
            let state = res.getState();
            if (state === "SUCCESS") {
                console.log("Successfully updated file types!");
            } else if (state === "INCOMPLETE") {
                //handle incompletion
            } else if (state === "ERROR") {
                console.log("Error updating file types...");
                let errors = res.getError();
                if (errors) {
                    if (errors[0] && errors[0].message) {
                        console.log("Error updating file types: " + errors[0].message);
                    }
                }
            } else {
                console.log("Unknown error whenever updating file types...");
            }
        });
